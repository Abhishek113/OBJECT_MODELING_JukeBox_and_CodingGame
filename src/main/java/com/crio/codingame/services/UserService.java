...
public class UserService implements IUserService {
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){
        ...
        if(allUsers.size() > 0)
        {
            Collections.sort(allUsers, User.getSortUserByScoresClass());
    ...
    public UserRegistrationDto withdrawContest(String contestId, String userName) throws ContestNotFoundException, UserNotFoundException, InvalidOperationException {
        ...
        if(contest == null)
            throw new ContestNotFoundException("Contest does not exist!");
        
        ContestStatus contestStatus = contest.getContestStatus();
        if(contestStatus.equals(ContestStatus.IN_PROGRESS) || contestStatus.equals(ContestStatus.ENDED))
            throw new InvalidOperationException("Contest is in progress!");
        ...
        if(user.equals(contest.getCreator()))
            throw new InvalidOperationException("Creater of contest can not withdraw contest!");
        
        boolean isUserRegisteredContest = user.checkIfContestExists(contest);
        if(!isUserRegisteredContest)
            throw new InvalidOperationException("User is not registered the given contest!");
